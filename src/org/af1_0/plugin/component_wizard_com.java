/*
 * component_wizard_com.java
 *
 * Created on 28 dicembre 2002, 13.43
 */
 ...
 * @author  Luca
 */
class component_wizard_com implements org.af1_0.gui.command.command {
    protected WizardDialogCOM wizard_dialog;
    protected multi_agent_system current_model;
    protected Vector lista_agenti_esterni;
    
    /** Creates a new instance of component_wizard_com */
    public component_wizard_com(WizardDialogCOM dialog,multi_agent_system project,Vector agenti_esterni) {
    ...
    public void execute() {
        ...
        wizard_panel.getPatternListPanel().setTitle("Component pattern list");
        wizard_panel.getPatternOverviewPanel().setTitle("Component pattern overview");
        wizard_panel.getSelectionPanel().setTitle("Agent target selection");
        ...
        wizard_panel.getSelectionPanel().setEntityList(agent_list);
        
        // 3.0 mostra la prima scheda
        component_wizard_com_sheet1 sheet1 = new component_wizard_com_sheet1( wizard_dialog, current_model,lista_agenti_esterni);        
        sheet1.execute();
    }
    ...
    private Vector preleva_pattern_da_repository() {
        ...
        Vector names = current_model.getRepository().queryComponentList();
        Iterator it = names.iterator();
        while (it.hasNext()) {
            String pattern_name = (String) it.next();
            pattern_list.add( current_model.getRepository().queryComponentItem(pattern_name) );
        }
